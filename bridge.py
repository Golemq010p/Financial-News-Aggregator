...
class FinancialJuiceWatcher:
    def __init__(self):
        ...
        self.info_token = ""
        self.cal_filters = Noneimport os
import requests
import json
import re
import time
            ...
            if res.status_code == 200:
                ...
                for ev in events:
                    ...
                    if not imp_val:
                        ...
                        if imp_id == '1': imp_val = 'High'
                        elif imp_id == '2': imp_val = 'Medium'
                        elif imp_id == '3': imp_val = 'Low'
                        else: imp_val = 'Low'

                    country_code = ev.get('CountryCode', ev.get('Country'))
                    if not country_code:
                        country_code = 'Global'
                    ...
                    processed_count += 1
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Calendar Sync: Processed {processed_count} events (out of {len(events)} total).")
            else:
